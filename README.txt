The J4R package is licensed under the GNU General Public License v3.0 (GPL-3). 

The J4R package makes it possible to execute native Java code in an R environment. More specifically, it allows to create Java objects and to invoke Java methods, whether they are static or not. The package is based on a local Java server that is instantiated through a Java library called REpicea. The REpicea library is licensed under the Lesser GNU General Public License v3.0 (LGPL-3). Both the J4R package and the REpicea library come with absolutely NO WARRANTY. 

The source code of the J4R package is freely available at https://sourceforge.net/p/repiceasource/code/HEAD/tree/trunk/ .
That of the REpicea library is also freely available at https://sourceforge.net/p/repiceasource/svn/HEAD/tree/trunk/ .

The documentation can be found at https://sourceforge.net/p/repiceasource/wiki/J4R/ .

Tickets can be created at https://sourceforge.net/p/repiceasource/tickets/ .

Mathieu Fortin 
mathieu.fortin.re@gmail.com
 
